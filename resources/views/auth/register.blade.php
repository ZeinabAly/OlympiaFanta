<x-guest-layout>
    <h2 class="section-title text-[2.1rem] font-extrabold text-center mb-10 text-green-950 !important">Inscription</h2>
    <form method="POST" action="{{ route('register') }}" class="space-y-4 w-full" >
        @csrf

        <!-- Name -->
        <div>
            <x-input-label for="name" :value="__('Nom')" placeholder="Fanta Sylla" class="block text-sm font-medium text-gray-700 mb-1" />
        ...
        <!-- Email Address -->
        <div class="mt-4">
            <x-input-label for="email" :value="__('Email')" placeholder="fanta@gmail.com" class="block text-sm font-medium text-gray-700 mb-1" />
        ...
        <div class="">
            <div class="mt-7 flex justify-center" >
                <button type="submit" class="w-full bg-[--color1-green] hover:bg-[#094018] border border-[--color1-green] hover:text-white text-white font-medium py-2.5 rounded-lg transition-colors">
                    S'inscrire
                </button>
            </div>
